...
class DialDetailsViewController: CreekBaseViewController {
    ...
    @objc func rightClick(){
        ...
        CreekInterFace.instance().encodeDial { model in
            CreekInterFace.instance().upload(withFileName: "\(self.titleName).bin", fileData: model) { progress in
               print(progress)
            } uploadSuccess: {
                print("uploadSuccess")
    ...
    func parseDial(){
        ...
        if let documentsURL = fileManager.urls(for: .documentDirectory, in: .userDomainMask).first {
            ...
            CreekInterFace.instance().parseDial(withPath: destinationURL.path, width: width, height: height, radius: cornerRadius,platformType: .jx3085CPlatform){ [weak self] model in
        ...
        if let model = dialParseModel{
            ...
            if(!(model.appColors).isEmpty){
                ...
                aview.callback = {(index) in
                    CreekInterFace.instance().setCurrentColorWithSelect(index) { [self]  model in
                        dispatch_main_sync_safe {
                            if let data = Data(base64Encoded: model.previewImageBytes, options: .ignoreUnknownCharacters) {
            ...
            if(!(model.backgroundImagePaths).isEmpty){
                ...
                aview.callback = {(index) in
                    CreekInterFace.instance().setCurrentBackgroundImagePathWithSelect(index) {[self] model in
                        dispatch_main_sync_safe {
                            if let data = Data(base64Encoded: model.previewImageBytes, options: .ignoreUnknownCharacters) {
            ...
            if(!(model.functions).isEmpty){
                ...
                aview.callBlock = {
                    ...
                    sheet.selectTypeBlock = { (selectIndexs)  in
                        ...
                        let list:[NSNumber] =  selectIndexs.map { e in
                            return NSNumber(value: e)
                        }
                        
                        CreekInterFace.instance().setCurrentFunctionWithSelectIndex(list) { model in
                            dispatch_main_sync_safe {
                                self.dialParseModel = model
