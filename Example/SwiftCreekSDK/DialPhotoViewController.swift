...
class DialPhotoViewController: CreekBaseViewController,CropViewControllerDelegate,UIImagePickerControllerDelegate & UINavigationControllerDelegate, TZImagePickerControllerDelegate {
    ...
    lazy var photoView :ZBSlideBackgroundPhotoView = {
        ...
        aview.callback = {
            ...
            self.dialParseModel?.photoSelectIndex =  NSNumber(value: $0)
            CreekInterFace.instance().setCurrentPhotoBackgroundImagePathWithPhotoImagePaths(self.dialParseModel?.photoImagePaths ?? [], select: self.dialParseModel?.photoSelectIndex.intValue ?? 0) { model in
    ...
    lazy var positionView :ZBSlideBackgroundView = {
        ...
        aview.callback = {
            CreekInterFace.instance().setCurrentClockPositionWithPhotoSelect(self.dialParseModel?.photoSelectIndex.intValue ?? 0, select: $0) { model in
                self.dialParseModel = model
                self.setDialUI()
            }
        }
    ...
    lazy var colorView:ZBSlideColorView = {
        ...
        aview.callback = {
           print($0)
            CreekInterFace.instance().setCurrentPhotoColorWithPhotoSelect(self.dialParseModel?.photoSelectIndex.intValue ?? 0, select: $0) { model in
                self.dialParseModel = model
                self.setDialUI()
            }
        }
    ...
    func setDialUI(){
        if let mode = dialParseModel {
            ...
            if !mode.colorSelectIndexList.isEmpty{
                colorView.slideBarItemSelected(index: mode.colorSelectIndexList[mode.photoSelectIndex.intValue].intValue)
            }
            ...
            if !mode.clockPositionSelectIndexList.isEmpty{
                positionView.slideBarItemSelected(index: mode.clockPositionSelectIndexList[mode.photoSelectIndex.intValue].intValue)
            }
            ...
            photoView.slideBarItemSelected(index: mode.photoSelectIndex.intValue)
    ...
    @objc func rightClick(){
        ...
        CreekInterFace.instance().encodePhotoDial { model in
            CreekInterFace.instance().upload(withFileName: "photo.bin", fileData: model) { progress in
               print(progress)
            } uploadSuccess: {
                print("uploadSuccess")
        ...
        let fileManager = FileManager()
           if let documentsURL = fileManager.urls(for: .documentDirectory, in: .userDomainMask).first {
               ...
               do {
                   ...
                   self.dialParseModel?.photoSelectIndex = NSNumber(value: (self.dialParseModel?.photoImagePaths.count ?? 0) - 1)
