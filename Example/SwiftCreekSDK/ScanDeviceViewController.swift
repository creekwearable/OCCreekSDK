...
class ScanDeviceViewController: CreekBaseViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let a = devides?[indexPath.row] as? ScanDeviceModel{
            ...
            if !a.macAddress.isEmpty{
                cell?.addressLab.text = a.macAddress
            }else{
                cell?.addressLab.text = a.device.identifier
            }
            if  (connectDeviceID != nil)  && CreekInterFace.instance().connectStatus == .connect {
                cell?.connectBtn.setTitle(connectDeviceID == a.device.identifier ? "disconnect" : "connect", for: .normal)
                cell?.connectBtn.backgroundColor = connectDeviceID == a.device.identifier ? .red : .black
            }
            cell?.connect = {
                self.view.showRemark(msg: "connect.....")
                CreekInterFace.instance().connect(a.device.identifier) { isBool in
    ...
    func scan(){
        CreekInterFace.instance().getBindDevice { model in
            print("开始扫描")
            CreekInterFace.instance().scan(withTimeOut: 15) { data in
                let filteredArray = data.filter { element in
                    return !model.contains { e in
                        return e.device.identifier == element.device.identifier
                    }
                }
